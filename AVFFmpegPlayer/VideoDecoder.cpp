...
AVController *VideoDecoder::initParameters( AVController * controller){
    ...
    //    else
    AVCodec * codec = avcodec_find_decoder(  controller->video_codec_ctx->codec_id); //作业： 硬件解码
    if(!codec) {
...
void VideoDecoder::resize(int w, int h)
{
    render->resize(w,h);
}
        ...
        if(!controller->video_frame_queue->isEmpty()){
            ...
            if(frame){
               int64_t pts_time= (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts;
                ...
                QImage resizedImage = image.scaled(render->size(),Qt::KeepAspectRatio, Qt::SmoothTransformation);
                render->updateRender(resizedImage);
                av_frame_free(&frame);
            }
