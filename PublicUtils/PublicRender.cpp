#include "PublicRender.h"

PublicRender::PublicRender(QObject *parent)
    : QObject{parent}
{}
bool PublicRender::InitSpeaker()
{
    qAudioFormat.setSampleRate(16000);
    qAudioFormat.setChannelCount(1);
    qAudioFormat.setSampleSize(16);
    qAudioFormat.setCodec("audio/pcm");
    qAudioFormat.setByteOrder(QAudioFormat::LittleEndian);
    qAudioFormat.setSampleType(QAudioFormat::SignedInt);

    out = new QAudioOutput(qAudioFormat);
    if (!out) {
        return false;
    }

    audioIO = out->start();
    return true;
}
void PublicRender::CloseSpeaker(){
    if (audioIO) {
        if (audioIO->isOpen()) audioIO->close();
        delete audioIO;    //qIODevice need to manual release, or will memory leak
        audioIO = nullptr;
    }

    if (out) {
        out->stop();
        delete out;
        out = nullptr;
    }
}
bool PublicRender::InitMicrophone()
{
    qIODevice = nullptr;
    format.setSampleRate(16000);
    format.setChannelCount(1);      // 设定声道数目，mono(平声道)的声道数目是1；stero(立体声)的声道数目是2
    format.setSampleSize(16);       // 采样位深
    format.setCodec("audio/pcm");   // 设置唯一支持的codec
    format.setByteOrder(QAudioFormat::LittleEndian);
    format.setSampleType(QAudioFormat::SignedInt);

    QAudioDeviceInfo info = QAudioDeviceInfo::defaultInputDevice();
    if (!info.isFormatSupported(format))
    {
        format = info.nearestFormat(format);
    }

    audioInput = new QAudioInput(format, nullptr);
    qIODevice = audioInput->start(); // 这里可以直接写入到文件，咱不用直接就是播放 audioInput->start(file);
    //qIODevice need to manual release, or will memory leak

    if (qIODevice) {
        qDebug() << "device available";
        return true;
    }

    return false;
}

void PublicRender::CloseMicrophone(){
    if (qIODevice) {
        if (qIODevice->isOpen()) qIODevice->close();
        delete qIODevice;    //qIODevice need to manual release, or will memory leak
        qIODevice = nullptr;
    }

    if (audioInput) {
        audioInput->stop();
        delete audioInput;
        audioInput = nullptr;
    }
}
QByteArray PublicRender::ReadAll(){
    if(qIODevice){
        return qIODevice->readAll();
    }
    return QByteArray();
}
void PublicRender::onReadyRead()
{

    QByteArray bytes= qIODevice->readAll();
    // qDebug() << "read pcm size" << bytes.size();
    qint64 bufferPlay = out->periodSize();

    if (out->bytesFree() < bufferPlay) { //
        qDebug() << "play buffer not enough";
        return;
    }
    audioIO->write((const char*)bytes.data(),bytes.size());

}

// void PublicRender::audioCaptureAndPlay()
// {
//     initQtAudioForPlay(); // for play
//     initInputFormat(); // for capture
//     connect(qIODevice, SIGNAL(readyRead()), this, SLOT(onReadyRead()));
// }


void PublicRender::WritePlay(QByteArray &bytes){
    if(bytes.isEmpty())return;
    if(audioIO){
        audioIO->write((const char*)bytes.data(),bytes.size());
    }
}

